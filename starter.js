...
const calculator = (num1, num2, callback) => {
    // we'll check to see if its true that num1 and num2 can be coerced into a number
    // if they are able to be numbers, we'll do it and call the callback to do our math
    // if they aren't able to be number, we warn them that they need to be numbers
        if (Number(num1) && Number(num2)) {
            ...
            console.log("Dawg, enter numbers not strings dummy")
        }
}
...
const applyDiscountsByCategory = (arr, callback, category, discount) => {
...
// applyDiscountsByCategory(dogProducts, applyPercentDiscount, 1, .60)
// console.log(dogProducts)

// applyDiscountsByCategory(catProducts, applyFlatRateDiscount, 2, .50)
// console.log(catProducts)

const applyDiscountsByInventory = (arr, callback, inventory, discount) => {
    for (let i=0; i<arr.length; i++) {
        if(arr[i].inventory >= 20){
            callback(arr[i], discount)
        }
    }
}

// applyDiscountsByInventory(catProducts, applyFlatRateDiscount, 3, .50)
// console.log(catProducts)
...
const productOfArray = (accumulator) {
    
}

// CODE HERE
const product = numsToReduce.reduce(productOfArray)
